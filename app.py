...
list_pelayanan_lainnya = ['', 'Makanan', 'Selimut', 'Bantal','Taksi', 'Asuransi']
...
with conn.session as session:
    query = text('CREATE TABLE IF NOT EXISTS STATION (id serial, nama text, gender text, \
                                                       tanggal_keberangkatan date, asal_stasiun text, tujuan_stasiun text, kelas text, nomor_kursi text, pelayanan_lainnya text);')
...
if page == "Edit Data":
    if st.button('Tambah Data'):
        with conn.session as session:
            query = text('INSERT INTO station (nama, gender, tanggal_keberangkatan, asal_stasiun, tujuan_stasiun, \
                          kelas, nomor_kursi, pelayanan_lainnya) VALUES (:1, :2, :3, :4, :5, :6, :7, :8);')
    ...
    for _, result in data.iterrows():        
        ...
        with st.expander(f'a.n. {nama_lama}'):
            with st.form(f'data-{id}'):
                ...
                pelayanan_lainnya_baru = st.multiselect("pelayanan_lainnya", ['Makanan', 'Selimut', 'Bantal', 'Taksi', 'Asuransi'], default=list(filter(lambda x: x in list_pelayanan_lainnya, eval(pelayanan_lainnya_lama))))
